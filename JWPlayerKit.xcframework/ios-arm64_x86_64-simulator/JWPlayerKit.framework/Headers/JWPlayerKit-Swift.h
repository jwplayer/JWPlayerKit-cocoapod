...
/// An object that can encode itself into a <code>JSONObject</code> representation.
...
@class NSURL;
@class JWAdOffset;
SWIFT_ENUM_FWD_DECL(NSInteger, JWAdType)
...
/// An object that can decode itself from a <code>JSONObject</code> representation.
SWIFT_PROTOCOL("_TtP11JWPlayerKit15JWJSONDecodable_")
@protocol JWJSONDecodable
/// Returns a new object from the native JSON representation object.
/// \param json A <code>JSONObject</code> (alias for <code>[String: Any]</code>) representing the required data to construct a valid object.
///
...
@interface JWAdBreakBuilder : NSObject <JWJSONDecodable>
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
/// Returns a <code>JWAdBreakBuilder</code> instance with initial values equal to a valid JSON representation of an ad break.
...
///     .build()
///
/// \endcode\param json The <code>JSONObject</code> from which the builder will copy values. Must be valid.
///
- (nullable instancetype)initFromJSON:(NSDictionary<NSString *, id> * _Nonnull)json;
...
SWIFT_ENUM_FWD_DECL(NSInteger, JWAdCompanionType)
...
SWIFT_ENUM_FWD_DECL(NSInteger, JWAdEventType)
SWIFT_ENUM_FWD_DECL(NSInteger, JWAdEventKey)
...
SWIFT_ENUM_FWD_DECL(NSInteger, JWAdPosition)
...
SWIFT_ENUM_FWD_DECL(NSInteger, JWAdShownOnSeek)
...
SWIFT_ENUM_FWD_DECL(NSInteger, JWAirPlayStatus)
...
SWIFT_ENUM_FWD_DECL(NSInteger, JWCaptionStyleOverrideStrategy)
...
SWIFT_PROTOCOL("_TtP11JWPlayerKit18JWChaptersDelegate_")
...
/// Button to exit fullscreen mode. Default to <code>JWVisibilityState.hidden</code>.
  JWControlTypeExitFullScreenButton = 10,
};

/// For objects where the decodable pattern is not available, a static factory method can vend an object.
SWIFT_PROTOCOL("_TtP11JWPlayerKit23JWInitializableFromJSON_")
@protocol JWInitializableFromJSON
/// A static factory method that vends a new object decoded from a <code>JSONObject</code> representation.
+ (nullable instancetype)fromJson:(NSDictionary<NSString *, id> * _Nonnull)json SWIFT_WARN_UNUSED_RESULT;
@end
...
@interface JWCue : NSObject <JWInitializableFromJSON, JWJSONDecodable, JWJSONEncodable>
...
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
/// Initializes a <code>JWCue</code> representation of a <code>JSONObject</code>.
/// The <code>JSONObject</code> is expected to have the following values. If required properties are not found within the <code>JSONObject</code>, this initializer returns <code>nil</code>.
...
- (nullable instancetype)initFromJSON:(NSDictionary<NSString *, id> * _Nonnull)json OBJC_DESIGNATED_INITIALIZER;
- (NSDictionary<NSString *, id> * _Nonnull)toJSONObject SWIFT_WARN_UNUSED_RESULT;
...
SWIFT_ENUM_FWD_DECL(NSInteger, JWFriendlyObstructionPurpose)
...
/// Convert a JSON array into an array of <code>JWPlayerItem</code> objects.
/// note:
...
/// \param jsonData The JSON data representing an array of playlist item objects.
///
///
/// throws:
/// An error if the json data can not be converted properly.
///
/// returns:
/// An array of <code>JWPlayerItem</code> used to load a playlist into a <code>JWPlayer</code>.
+ (NSArray<JWPlayerItem *> * _Nullable)playlistItemsFromJSONData:(NSData * _Nonnull)jsonData error:(NSError * _Nullable * _Nullable)error SWIFT_WARN_UNUSED_RESULT;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end
...
@class UIImage;
SWIFT_ENUM_FWD_DECL(NSInteger, JWLogoPosition)
...
SWIFT_ENUM_FWD_DECL(NSInteger, JWTrackType)
...
SWIFT_ENUM_FWD_DECL(NSInteger, JWPlayerState)
...
/// Updates the metadata of the currently playing item without interrupting playback.
/// Use this method to dynamically change the title, description, and poster image for live streams
/// (e.g., radio stations) when the content segment changes. Only non-nil parameters are applied.
/// \param title The new title, or <code>nil</code> to leave unchanged.
///
/// \param description The new description, or <code>nil</code> to leave unchanged.
///
/// \param posterImage The new poster image URL, or <code>nil</code> to leave unchanged.
///
- (void)updateItemMetadataWithTitle:(NSString * _Nullable)title description:(NSString * _Nullable)description posterImage:(NSURL * _Nullable)posterImage;
...
SWIFT_ENUM_FWD_DECL(NSInteger, JWPreload)
...
@interface JWPlayerConfigurationBuilder : NSObject <JWJSONDecodable>
/// Returns a <code>JWPlayerConfigurationBuilder</code> instance with initial values equal to a valid JSON representation of a player configuration.
/// These values can still be subsequently assigned. For example, to overwrite a player’s autostart setting, retaining all other values:
/// \code
/// let playerConfig = try! JWPlayerItemBuilder(from: sourcePlayerConfigurationJsonObject)
///     .autostart(true)
///     .build()
///
/// \endcode\param json The <code>JSONObject</code> from which the builder will copy values. Must be valid.
...
@interface JWPlayerItem : NSObject <JWInitializableFromJSON, JWJSONEncodable>
...
@interface JWPlayerItemBuilder : NSObject <JWJSONDecodable>
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
...
/// Returns a <code>JWPlayerItemBuilder</code> instance with initial values equal to a valid JSON representation of a player item.
...
///     .build()
///
/// \endcode\param json The <code>JSONObject</code> from which the builder will copy values. Must be valid, containing either a <code>file</code> entry, or <code>sources</code> array entry.
///
- (nullable instancetype)initFromJSON:(NSDictionary<NSString *, id> * _Nonnull)json;
...
@optional
/// Reports when the metadata of the currently playing item has been updated via <code>JWPlayerProtocol/updateItemMetadata(title:description:posterImage:)</code>.
/// \param player The player emitting the event.
///
/// \param item The player item whose metadata was updated.
///
- (void)jwplayer:(id <JWPlayer> _Nonnull)player didUpdateItemMetadataForItem:(JWPlayerItem * _Nonnull)item;
@end

@class AVPictureInPictureController;
SWIFT_ENUM_FWD_DECL(NSInteger, JWVideoGravity)
...
SWIFT_ENUM_FWD_DECL(NSInteger, JWRelatedMethod)
/// A protocol listening for related events.
SWIFT_PROTOCOL("_TtP11JWPlayerKit22JWRelatedEventDelegate_")
@protocol JWRelatedEventDelegate
...
@protocol JWPlayerViewControllerFullScreenDelegate;
@protocol JWPlayerViewControllerUIDelegate;
@protocol JWPlayerViewControllerRelatedDelegate;
...
@interface JWPlayerViewController : UIViewController <AVPictureInPictureControllerDelegate, JWAVDelegate, JWAccessLogMetadataDelegate, JWAdDelegate, JWAirPlayDelegate, JWCastDelegate, JWChaptersDelegate, JWDateRangeMetadataDelegate, JWExternalMetadataDelegate, JWID3MetadataDelegate, JWMediaMetadataDelegate, JWPlayerDelegate, JWPlayerStateDelegate, JWPlayerViewDelegate, JWProgramDateTimeMetadataDelegate, JWRelatedEventDelegate, JWTimeEventListener, UIPopoverPresentationControllerDelegate>
...
@property (nonatomic, weak) id <JWPlayerViewControllerFullScreenDelegate> _Nullable fullScreenDelegate;
/// The delegate to receive JWPlayerViewControllerUI events.
@property (nonatomic, weak) id <JWPlayerViewControllerUIDelegate> _Nullable uiDelegate;
/// The delegate to receive JWPlayerViewControllerRelated events.
@property (nonatomic, weak) id <JWPlayerViewControllerRelatedDelegate> _Nullable relatedDelegate;
...
SWIFT_ENUM_FWD_DECL(NSInteger, JWVisibilityState)
@class NSNumber;
SWIFT_UNAVAILABLE
...
SWIFT_PROTOCOL("_TtP11JWPlayerKit40JWPlayerViewControllerFullScreenDelegate_")
@protocol JWPlayerViewControllerFullScreenDelegate
...
SWIFT_PROTOCOL("_TtP11JWPlayerKit37JWPlayerViewControllerRelatedDelegate_")
@protocol JWPlayerViewControllerRelatedDelegate
...
/// \param method How the related item began playing.
///
- (void)playerViewController:(JWPlayerViewController * _Nonnull)controller relatedItemBeganPlaying:(JWPlayerItem * _Nonnull)item atIndex:(NSInteger)index withMethod:(enum JWRelatedMethod)method;
@end

/// UI  events within the JWPlayerViewController are routed to this delegate.
SWIFT_PROTOCOL("_TtP11JWPlayerKit32JWPlayerViewControllerUIDelegate_")
@protocol JWPlayerViewControllerUIDelegate
...
SWIFT_ENUM_FWD_DECL(NSInteger, JWRelatedOnComplete)
SWIFT_ENUM_FWD_DECL(NSInteger, JWRelatedOnClick)
...
/// The number of seconds of content buffered ahead of the current playback position.
/// A value of <code>-1</code> indicates that buffer health data is not available.
/// For live streams where <code>duration</code> is not meaningful, this provides a reliable measure of buffer health.
@property (nonatomic) NSTimeInterval bufferHealth;
- (BOOL)isEqual:(id _Nullable)object SWIFT_WARN_UNUSED_RESULT;
...
SWIFT_ENUM_FWD_DECL(NSInteger, JWVisualQualityReason)
SWIFT_ENUM_FWD_DECL(NSInteger, JWVisualQualityMode)
